...
APP_VERSION = "v3.2-MDOF-Storywise"
...
def run_iterative_design(inp: BuildingInput) -> DesignResult:
    ...
    for iteration in range(1, max_iterations + 1):
        ...
        core_stiffness_share = K_core / K_total if K_total > 0 else 0.7
...
# ----------------------------- STORYWISE EXTENSIONS -----------------------------
@dataclass
class StorySectionResult:
    story: int
    zone_name: str
    elevation_m: float
    core_wall_thickness_m: float
    core_Ieff_m4: float
    corner_column_x_m: float
    corner_column_y_m: float
    perimeter_column_x_m: float
...
def zone_lookup_by_story(inp: BuildingInput) -> dict[int, ZoneDefinition]:
    zones = define_three_zones(inp.n_story)
    lookup: dict[int, ZoneDefinition] = {}
    for z in zones:
        for story in range(z.story_start, z.story_end + 1):
            lookup[story] = z
    return lookup


def story_taper_factor(zone: ZoneDefinition, story: int) -> float:
    if zone.n_stories <= 1:
        return 1.0
    rel = (story - zone.story_start) / max(zone.n_stories - 1, 1)
    return 1.0 - 0.08 * rel


def generate_storywise_member_schedule(
    inp: BuildingInput,
    zone_cores: List[ZoneCoreResult],
    zone_cols: List[ZoneColumnResult],
    beam_b: float,
    beam_h: float,
    slab_t: float,
) -> List[StorySectionResult]:
    core_by_zone = {z.zone.name: z for z in zone_cores}
    col_by_zone = {z.zone.name: z for z in zone_cols}
    story_zone = zone_lookup_by_story(inp)
    ...
    E = inp.Ec * 1e6
    h = inp.story_height
    story_results: List[StorySectionResult] = []

    for story in range(1, inp.n_story + 1):
        zone = story_zone[story]
        zc = core_by_zone[zone.name]
        zcol = col_by_zone[zone.name]
        taper = story_taper_factor(zone, story)

        t_wall = max(inp.min_wall_thickness, min(inp.max_wall_thickness, zc.wall_thickness * taper))
        I_core = max(zc.Ieq_effective_m4 * taper**3, 1e-9)

        cx = max(inp.min_column_dim, min(inp.max_column_dim, zcol.corner_column_x_m * taper))
        cy = max(inp.min_column_dim, min(inp.max_column_dim, zcol.corner_column_y_m * taper))
        px = max(inp.min_column_dim, min(inp.max_column_dim, zcol.perimeter_column_x_m * taper))
        py = max(inp.min_column_dim, min(inp.max_column_dim, zcol.perimeter_column_y_m * taper))
        ix = max(inp.min_column_dim, min(inp.max_column_dim, zcol.interior_column_x_m * taper))
        iy = max(inp.min_column_dim, min(inp.max_column_dim, zcol.interior_column_y_m * taper))

        I_corner = max(cx * cy**3 / 12.0, cy * cx**3 / 12.0)
        I_perim = max(px * py**3 / 12.0, py * px**3 / 12.0)
        I_inter = max(ix * iy**3 / 12.0, iy * ix**3 / 12.0)
        I_col_total = corner_cols * I_corner + perimeter_cols * I_perim + interior_cols * I_inter

        # Story stiffness based on member flexural stiffness summation.
        k_core_story = 12.0 * E * I_core / max(h**3, 1e-9)
        k_col_story = 12.0 * E * inp.column_cracked_factor * I_col_total / max(h**3, 1e-9)
        k_story = k_core_story + k_col_story
...
def equivalent_building_stiffness_from_story_stiffness(story_stiffness: List[float]) -> float:
    inv_sum = sum(1.0 / max(k, 1e-9) for k in story_stiffness)
    return 1.0 / max(inv_sum, 1e-18)


def build_story_stiffnesses(inp: BuildingInput, story_results: List[StorySectionResult]) -> List[float]:
    return [s.story_stiffness_N_per_m for s in story_results]


def solve_mdof_modes(inp: BuildingInput, total_weight_kN_value: float, story_stiffness: List[float], n_modes: int = 5) -> ModalResult:
    masses = build_story_masses(inp, total_weight_kN_value)
    k_stories = story_stiffness
    M, K = assemble_m_k_matrices(masses, k_stories)
    A = np.linalg.inv(M) @ K
    eigvals, eigvecs = np.linalg.eig(A)
    eigvals = np.real(eigvals)
...
def evaluate_design(inp: BuildingInput, core_scale: float, column_scale: float, beta: float):
    ...
    story_results = generate_storywise_member_schedule(inp, zone_cores, zone_cols, beam_b, beam_h, slab_t)
    story_stiffness = build_story_stiffnesses(inp, story_results)

    K_core = weighted_core_stiffness(inp, zone_cores)
    K_cols = weighted_column_stiffness(inp, zone_cols)
    K_est = equivalent_building_stiffness_from_story_stiffness(story_stiffness)

    modal = solve_mdof_modes(inp, W_total, story_stiffness, n_modes=5)
    T_est = modal.periods_s[0] if modal.periods_s else 2.0 * pi * sqrt(M_eff / max(K_est, 1e-9))
...
def run_iterative_design(inp: BuildingInput) -> DesignResult:
    ...
    for iteration in range(1, max_iterations + 1):
        ...
        stiffness_ratio = (T_est / T_target) ** 2
        core_stiffness_share = K_core / (K_core + K_cols) if (K_core + K_cols) > 0 else 0.7
        damping = 0.65
        ...
        scale_change = abs(new_core_scale - core_scale) + abs(new_column_scale - column_scale)
        if scale_change < 0.002 and iteration > 3 and error > tolerance:
            if core_stiffness_share > 0.5:
                new_core_scale = max(MIN_SCALE, core_scale * 0.95)
            else:
                new_column_scale = max(MIN_SCALE, column_scale * 0.95)
    ...
    if best_result is None or best_error > tolerance:
        res, core_scale, column_scale, ev = optimize_scales(inp, beta, x0=np.array([core_scale, column_scale]))
    else:
        core_scale, column_scale, ev = best_result
    ...
    messages = [
        ...
        f"T_upper = {T_upper:.3f} s",
        f"T_target = {T_target:.3f} s",
        f"Final T_est (storywise MDOF) = {T_est:.3f} s",
        f"Iterations used = {len(iteration_history)}",
        f"MDOF Mode 1 mass participation = {100*ev['modal'].effective_mass_ratios[0]:.1f}%",
        "Story stiffness is assembled from floor-by-floor wall and column flexural stiffness.",
    ]
    messages.append("Upper period check = OK" if period_ok else "Upper period check = NOT OK")
    messages.append("Drift check = OK" if drift_ok else "Drift check = NOT OK")
    ...
    result = DesignResult(
        ...
        reinforcement=ev["reinf"],
        optimization_success=True,
        optimization_message="Iterative storywise MDOF convergence",
        core_scale=core_scale,
        column_scale=column_scale,
        messages=messages,
        redesign_suggestions=redesign_suggestions,
    ...
    result.story_results = ev["story_results"]
    return result


def story_results_dataframe(result: DesignResult) -> pd.DataFrame:
    story_results = getattr(result, "story_results", [])
    return pd.DataFrame([
        {
            "Story": s.story,
            "Zone": s.zone_name,
            "Elevation (m)": round(s.elevation_m, 2),
            "Wall t (m)": round(s.core_wall_thickness_m, 3),
            "Core Ieff (m4)": round(s.core_Ieff_m4, 3),
            "Corner Col (m)": f"{s.corner_column_x_m:.2f} x {s.corner_column_y_m:.2f}",
            "Perim Col (m)": f"{s.perimeter_column_x_m:.2f} x {s.perimeter_column_y_m:.2f}",
            "Interior Col (m)": f"{s.interior_column_x_m:.2f} x {s.interior_column_y_m:.2f}",
            "Beam (m)": f"{s.beam_width_m:.2f} x {s.beam_depth_m:.2f}",
            "Slab t (m)": round(s.slab_thickness_m, 3),
            "Story K (MN/m)": round(s.story_stiffness_N_per_m / 1e6, 3),
        }
...
def build_report(result: DesignResult) -> str:
    ...
    story_results = getattr(result, "story_results", [])
    if story_results:
        lines.append("STORY-BY-STORY MEMBER SCHEDULE")
        lines.append("-" * 74)
        lines.append(f"{'Story':>5} {'Zone':>12} {'Wall t':>8} {'Corner col':>18} {'Perim col':>18} {'Interior col':>18} {'Story K(MN/m)':>14}")
        for s in story_results:
            lines.append(f"{s.story:>5} {s.zone_name[:12]:>12} {s.core_wall_thickness_m:>8.3f} {f'{s.corner_column_x_m:.2f}x{s.corner_column_y_m:.2f}':>18} {f'{s.perimeter_column_x_m:.2f}x{s.perimeter_column_y_m:.2f}':>18} {f'{s.interior_column_x_m:.2f}x{s.interior_column_y_m:.2f}':>18} {s.story_stiffness_N_per_m/1e6:>14.2f}")
        lines.append("")
...
st.title("Final Tall Building Plan Output Tool + Storywise MDOF Iterative Loop")
...
with right_col:
    ...
    if st.session_state.result is None:
        ...
        tab1, tab2, tab3, tab4, tab5 = st.tabs(["Graphic output", "Convergence Plot", "Mass participation", "Story schedule", "Report"])
        ...
        with tab4:
            story_df = story_results_dataframe(res)
            st.dataframe(story_df, use_container_width=True, hide_index=True, height=520)
            csv_data = story_df.to_csv(index=False).encode("utf-8")
            st.download_button("Download story schedule CSV", data=csv_data, file_name="story_schedule.csv", mime="text/csv")
