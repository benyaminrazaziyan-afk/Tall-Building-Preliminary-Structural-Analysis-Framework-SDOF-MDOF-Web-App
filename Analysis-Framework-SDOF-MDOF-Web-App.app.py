...
def total_dynamic_height(inp: BuildingInput) -> float:
...
def total_weight_and_mass(inp: BuildingInput, slab_t: float) -> Tuple[float, float]:
    ...
    q_above = inp.DL + inp.LL + inp.slab_finish_allowance + slab_self
    q_base = 1.10 * q_above
    W = q_above * area * inp.n_story
    W += q_base * area * inp.n_basement
...
def build_story_masses(inp: BuildingInput, total_mass: float) -> List[float]:
    # distribute mass to all dynamic stories, including basements
    n_total = inp.n_story + inp.n_basement
    if n_total <= 0:
        return []
    above_weight = inp.n_story
    basement_weight = 1.10 * inp.n_basement
    denom = above_weight + basement_weight if (above_weight + basement_weight) > 0 else 1.0
    m_above = total_mass * (1.0 / denom)
    m_base = total_mass * (1.10 / denom)
    return [m_base for _ in range(inp.n_basement)] + [m_above for _ in range(inp.n_story)]


def basement_stiffness_factor(inp: BuildingInput) -> float:
    # stronger basement stories due to retaining walls and soil confinement
    t = inp.basement_retaining_wall_thickness
    factor = 2.0 + 3.0 * min(max(t / 0.50, 0.5), 2.0)
    return float(np.clip(factor, 2.0, 8.0))
...
def build_story_stiffnesses(inp: BuildingInput, K_total: float) -> List[float]:
    ...
    raw = []
    base_factor = basement_stiffness_factor(inp)
    for i in range(n_total):
        if i < inp.n_basement:
            # basement stories are stiffer because of retaining walls
            raw.append(base_factor)
        else:
            j = i - inp.n_basement
...
def run_design(inp: BuildingInput) -> DesignResult:
    ...
    if inp.n_basement > 0:
        sugg.insert(0, "Dynamic model includes basement stories and retaining-wall stiffness in basement levels.")
